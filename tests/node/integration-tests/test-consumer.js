...
var fixtureEKeyBlob;
var fixtureDKeyBlob;
var fixtureUEKeyBlob;
var fixtureUDKeyBlob;
...
{
  var contentData = new Data(contentName);
  var encryptParams = new EncryptParams(EncryptAlgorithmType.AesCbc);
...
function createEncryptedCKey()
{
  var cKeyData = new Data(cKeyName);
...
function createEncryptedDKey()
{
  var dKeyData = new Data(dKeyName);
  var encryptParams = new EncryptParams(EncryptAlgorithmType.RsaOaep);
  Encryptor.encryptData
...
describe ("TestConsumer", function() {
  ...
  it("DecryptContent", function(done) {
    ...
    var aesKeyBlob = new Blob(AES_KEY, false);
    ...
    // Decrypt.
    Consumer.decryptPromise_(cKeyData.getContent(), fixtureDKeyBlob)
    .then(function(result) {
      assert.ok(result.equals(aesKeyBlob));

      // Decrypt.
      return Consumer.decryptPromise_(contentData.getContent(), fixtureCKeyBlob);
    })
    .then(function(result) {
      assert.ok(result.equals(new Blob(DATA_CONTENT, false)));
